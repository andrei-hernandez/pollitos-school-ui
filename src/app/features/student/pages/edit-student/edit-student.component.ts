...
export class EditStudentComponent implements OnInit {
  ...
  loadStudentData(): void {
    this.isLoading = true
    this.studentService.getStudentById(this.studentId!, this.institution).subscribe({
      next: (student) => {
  ...
  onSubmit(): void {
    if (this.studentForm.valid && this.studentId) {
      ...
      this.studentService.updateStudent(this.studentId, updatedStudent, this.institution).subscribe({
        next: () => {
